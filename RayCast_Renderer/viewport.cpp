...
//void showMaterialName(const Material *m, HitInfo &hInfo)
//{
//	const char *materialName = m->GetName();
//	printf(" %s", materialName);
//}

Ray ComputeCameraRay(int i, int j)
{
	float l = 1.0f / tan((camera.fov*M_PI / 180) / 2.0f);
	Point3 camera_right = camera.dir.Cross(camera.up);
	camera_right = camera_right.GetNormalized();
	const float width = camera.imgWidth;
	const float height = camera.imgHeight;
	float aspect = width / height;

	//Calculating (u,v)
	double u = 2 * ((i + 0.5) / width)*aspect - aspect;
	double v = 2 * ((j + 0.5) / height) - 1;
	Point3 image_point = (u*camera_right) +
		(v * camera.up) +
		camera.pos + (camera.dir*l);
	Point3 ray_direction = image_point - camera.pos;
	//normalize direction
	ray_direction = ray_direction.GetNormalized();
